...
**Baseline Statistics:**
- Stored in `models/baseline_feature_stats.parquet`
...
drift_report = detect_drift(
    current_features,
    baseline_stats,
    threshold=0.15  # 15% distribution shift
)

if drift_report['drift_detected']:
    print(f"⚠️ Drift detected in features: {drift_report['drifted_features']}")
...
### Production Monitoring Dashboard (Conceptual)

For real production deployment, integrate with:
...
### Scaling for Real Fleet Operations

**Current Implementation (Redis):**
- ✅ Lightweight feature store demonstration
- ✅ Suitable for: 50-500 vehicles
- ✅ Last-N-minute telemetry buffer per vehicle

**Production-Scale Architecture (Kafka + Feature Store):**

```
Vehicle Telemetry (1000s of vehicles)
          ↓
    Kafka Topic (per vehicle partition)
          ↓
  Streaming Feature Builder (Kafka Streams / Flink)
          ↓
    Feature Store (Feast / Tecton)
          ↓
  Inference API (FastAPI + Redis Cache)
          ↓
    Dashboard / Alerts (Grafana / PagerDuty)
```

**Why Kafka for Production:**
- ✅ **Reliable ingestion** with exactly-once semantics
- ✅ **Buffering & replay** for backfilling or debugging
- ✅ **Ordering guarantees** per vehicle (partition key)
- ✅ **Multiple consumers** (real-time + batch pipelines)
- ✅ **Scalability** to millions of messages/sec
...
2. **Data Drift Detected:** Feature distribution shift > 15%
3. **Time-based:** Retrain every 30 days minimum
4. **New Vehicle Models:** When fleet composition changes
...
python run_features.py --mode production

# 3. Retrain with Optuna
